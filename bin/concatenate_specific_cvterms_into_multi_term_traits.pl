...
Will produce cvterms of form: harvest index|CO:0000015||cass week 1|CASSTIME:0000006
...
foreach my $i (@first_parent_names) {
    ...
    foreach my $a (@$first_term) {
        ...
        foreach my $b (@$second_term) {
            ...
            push @concatenated_terms, $b_concat_term;
        }
    }

    print Dumper \@concatenated_terms;
    print scalar(@{$children_array[0]}) * scalar(@{$children_array[1]}) ."\n";
