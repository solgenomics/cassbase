...
if [ "$export_type" == "6" ]; then
    wget --no-check-certificate --output-document=${processing_file_dir}/phenotype_download.csv "${host_address}/breeders/trials/phenotype/download?trial_list=${trial_ids}&format=csv&timestamp=0&trait_list=${trait_ids}&accession_list=${accession_ids}&dataLevel=plot&sgn_session_id=${sgn_session_id}&has_header=0"
    ...
    wget --no-check-certificate --output-document=${processing_file_dir}/phenotype_download.csv "${host_address}/breeders/trials/phenotype/download?trial_list=${trial_ids}&format=csv&timestamp=0&trait_list=${trait_ids}&accession_list=${accession_ids}&dataLevel=plant&sgn_session_id=${sgn_session_id}&has_header=0"
